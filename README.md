...
- `financial_dashboard.html` — 인터랙티브 비교 대시보드. 상단 검색창에 회사명을 입력하면 `api/search.js`가 국내 상장기업 3,976개 전체에서 실시간으로 찾아주고, 선택하면 `api/company.js`가 DART에서 그 기업의 최근 3개년 재무제표·배당·분리과세 판정을 즉시 조회합니다. 삼성전자/SK하이닉스/한국전력 등 8개 기업은 미리 채워진 값으로 즉시 표시되고, 그 외 기업은 실시간 조회됩니다.
  - 분리과세 판정 배지의 `▶`를 누르면 조세특례제한법 제104조의27의 조건별(①상장여부/②비감소/③-가 40%/③-나 25%+10%증가) 충족 여부와 실제 수치를 볼 수 있습니다.
  - 조세특례제한법 시행령(2026.2.24. 국무회의 의결)에 따라 고배당기업이 정기주주총회 이익배당 결의 다음 날까지 한국거래소 상장공시제출시스템에 제출해야 하는 "기업가치 제고계획" 법정 의무공시(공시명에는 "(자율공시)"로 표기되나 고배당기업에게는 의무)를 KIND(kind.krx.co.kr)에서 실시간 조회해, 회사가 직접 공시한 배당성향·증가율과 이 도구의 계산이 일치하는지 교차검증합니다 (`api/company.js`의 `checkKrxDisclosure`).
