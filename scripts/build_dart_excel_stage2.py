...
with open(os.path.join(HERE, "row_map.json"), encoding="utf-8") as f:
    ROW_MAP = json.load(f)

COMPANIES = ["삼성전자", "SK하이닉스", "현대자동차"]
YEAR_COLS = ["B", "C", "D"]
YEARS = ROW_MAP[COMPANIES[0]]["years"]  # [2023, 2024, 2025]

wb = load_workbook(os.path.join(HERE, "재무비교분석기_stage1.xlsx"))
RAW = "원본데이터"
...
# ratio definitions: (label, numerator_key, denominator_key, fmt, growth=False)
RATIO_SECTIONS = [
    ("[ 수익성 지표 ]", [
...
ratio_row_map = {}  # (metric_label) -> {company: row}
...
# growth section (only 2024, 2025 columns valid; 2023 = N/A, no prior period)
GROWTH_DEFS = [
    ("매출액증가율", "revenue"),
    ("영업이익증가율", "op_income"),
    ("순이익증가율", "net_income"),
...
for label, key in GROWTH_DEFS:
    ...
    for company in COMPANIES:
        rm = ROW_MAP[company]
        ws2.cell(row=r, column=1, value=company).font = BLACK
        # B column (oldest year) has no prior period available -> N/A
        ws2[f"B{r}"] = "N/A"
...
with open(os.path.join(HERE, "ratio_row_map.json"), "w", encoding="utf-8") as f:
    json.dump(ratio_row_map, f, ensure_ascii=False, indent=2)

wb.save(os.path.join(HERE, "재무비교분석기_stage2.xlsx"))
print("stage2 saved")
print(json.dumps(ratio_row_map, ensure_ascii=False, indent=2))
