...
DIVIDEND_ALIASES = {"cash_dividend_total": ["현금배당금총액"]}
...
def main():
    ...
    for display_name, dart_name in COMPANIES.items():
        ...
        print(f"[OK] {display_name} ({corp_name}) corp_code={corp_code} year={year} fs_div={fs_div} div_status={div_data.get('status')}")

    with open(os.path.join(HERE, "dart_data3.json"), "w", encoding="utf-8") as f:
        json.dump(result, f, ensure_ascii=False, indent=2)
    print("saved dart_data3.json")
