...
with open(os.path.join(HERE, "row_map.json"), encoding="utf-8") as f:
    ROW_MAP = json.load(f)
with open(os.path.join(HERE, "ratio_row_map.json"), encoding="utf-8") as f:
...
wb = load_workbook(os.path.join(HERE, "재무비교분석기_stage2.xlsx"))
ws_raw = wb[RAW]
ws_cmp = wb[CMP]
...
ws3["A1"] = "3개 기업 재무비교 대시보드 (삼성전자 / SK하이닉스 / 현대자동차)"
...
def add_line_chart(anchor, title, metric_key, y_fmt="#,##0"):
    chart = LineChart()
    chart.title = title
    chart.style = 2
...
def add_ratio_bar_chart(anchor, title, ratio_label):
    ...
    cat_row = ROW_MAP[COMPANIES[0]]["header"]  # reuse year header values 2023/2024/2025
    cats = Reference(ws_cmp, min_col=2, max_col=4, min_row=list(rows.values())[0] - 1, max_row=list(rows.values())[0] - 1)
    for company in COMPANIES:
        row = rows[company]
...
wb.save(os.path.join(HERE, "재무비교분석기.xlsx"))
print("stage3 (final) saved")
