...
lines = [
    ...
    "   - 이 시트는 2025년 세제개편안(조세특례제한법 개정, §91의19 신설 논의)에 대한 보도자료·법무법인 뉴스레터를",
    "     근거로 요건을 구성한 것입니다. 국회 통과 이후 최종 조문·시행령 세부 요건(세율 구간 등)은 다를 수 있습니다.",
...
ws4["A2"] = "2026.1.1. 이후 귀속 배당분부터 3년 한시 시행 예정 (조세특례제한법 개정안 §91의19 기준, 2026-07 시점 조사)"
ws4["A2"].font = Font(name=FONT_NAME, italic=True, color="808080")

disclaimer_lines = [
    "⚠ 주의: 이 판정은 확정된 법령이 아니라 2025년 세제개편안 보도 및 법무법인 뉴스레터를 근거로 구성한 추정치입니다.",
    "  - 요건(추정): ① 전년 대비 현금배당액 비감소, AND ② 배당성향 40% 이상 OR (배당성향 25% 이상 AND 직전 3년 평균 대비 배당금 5% 이상 증가)",
    "  - 배당성향은 각 기업이 DART에 직접 공시한 (연결)현금배당성향(%)을 사용합니다 (원본데이터 시트의 자체 계산 순이익 기준과 소수 오차 있을 수 있음).",
    "  - 세율 구간 등 세부 사항은 자료마다 상이하게 보도되어 이 표에 포함하지 않았습니다.",
...
for company in COMPANIES:
    ...
    condA_row = r
    ws4.cell(row=r, column=1, value="① 전년대비 배당 비감소").font = BLACK
    ...
    avg3_row = r
    ws4.cell(row=r, column=1, value="직전 3년(연도-3~연도-1) 평균 배당금").font = BLACK
    ws4[f"E{r}"] = f'=IF(COUNT(B{div_row}:D{div_row})=3,AVERAGE(B{div_row}:D{div_row}),"자료부족")'
    ws4[f"E{r}"].font = BLACK
    ws4[f"E{r}"].number_format = NUM_FMT
    ws4[f"E{r}"].border = BORDER
    r += 1

    growth_row = r
    ws4.cell(row=r, column=1, value="3년평균 대비 증가율").font = BLACK
    ws4[f"E{r}"] = f'=IF(ISNUMBER(E{avg3_row}),IFERROR(E{div_row}/E{avg3_row}-1,"-"),"-")'
    ws4[f"E{r}"].font = BLACK
    ...
    condB_row = r
    ws4.cell(row=r, column=1, value="② 배당성향 40%+ 또는 (25%+ 및 3년평균 5%+ 증가)").font = BLACK
    ws4[f"E{r}"] = (f'=IF(NOT(ISNUMBER(E{pay_row})),"판단불가",'
                     f'IF(E{pay_row}>=0.4,"충족(40%+)",'
                     f'IF(AND(E{pay_row}>=0.25,ISNUMBER(E{growth_row}),E{growth_row}>=0.05),'
                     f'"충족(25%+ 및 3년평균5%+증가)","미충족")))')
    ws4[f"E{r}"].font = BLACK
    ws4[f"E{r}"].border = BORDER
    r += 1
